			...
			mui(document).on('tap', '#goToLogin', function() {
					...
					get_loc({
						success: function(result) {
							if(result.addressComponent.adcode == null || result.addressComponent.adcode == undefined ||
								result.addressComponent.adcode == '' || result.addressComponent.adcode.length !== 6) { //数据无效
								var pageObj = {
		...
		getGpsPos: function() { //定位
			...
			if(cityId == null || cityId == undefined ||
				...
				get_loc({
					success: function(result) {
						if(result.addressComponent.adcode == null || result.addressComponent.adcode == undefined ||
							result.addressComponent.adcode == '' || result.addressComponent.adcode.length !== 6) {
							var pageObj = {
								pageUrl: 'page/location.html'
							};
							pageChange(pageObj);
							...
							page.init();
							locsaveuserinfo('citycode', result.addressComponent.adcode);
						}
					},
					error: function(e) {
						console.log('地址获取异常:' + JSON.stringify(e));
